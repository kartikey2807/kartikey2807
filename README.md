...
<div align="justify">
I recently completed my master's in data science, specializing in machine learning and statistics, from Uppsala University, and have hands-on experience in generative modeling (LLMs, GANs, VAEs, etc.), privacy-based ML, and distributed learning in edge networks. I did my master's thesis at Scania, where I implemented a <i>transformer-based variational autoencoder</i> to identify anomalies in truck components for predictive maintenance and trained the model using online and federated learning to improve generalizability and communication efficiency while satisfying memory constraints. I also collaborated with researchers from Uppsala University to build robustness into FL training by using a <i>distributed WGAN</i> model to detect and filter malicious clients (with data or gradient poisoning) during aggregation.
</div>
<br>
